...
* *Search Deleted*: Execute the search, but specifically for deleted results.
...
<<_deleting_a_record,Delete>>:: Remove the selected record from standard search results.
...
To remove a record from active search results, delete it.
...
Restore an deleted record to return it to active search results.

. Select the *Search Deleted* option from the query options menu.
. Select the desired record from the *Results* tab.
. Select *Delete*.
. Confirm restoring the record.

Restore all deleted records to active search results.
...
Customize the look and feel of the ${catalog-ui} using the *Settings* (image:gear-icon.png[settings]) menu on the Navigation Bar.
