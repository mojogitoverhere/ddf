  ...
  public void bTestAppStatus() throws ApplicationServiceException, InterruptedException {
    ...
    systemSubject.execute(
        () -> {
          ...
          assertThat(
              "Application [" + CATALOG_APP + "] should be ACTIVE", status.getState(), is(ACTIVE));

          List<Application> solrList =
              apps.stream().filter(a -> SOLR_APP.equals(a.getName())).collect(Collectors.toList());
          if (catalogList.size() != 1) {
          ...
          assertThat(
              "Application [" + SOLR_APP + "] should be INACTIVE", status.getState(), is(INACTIVE));
  ...
  public void cTestAppStartStop() throws ApplicationServiceException {
    ...
    systemSubject.execute(
        () -> {
          ...
          assertThat(
              SOLR_APP + " should be empty response after " + START_COMMAND,
              response,
              isEmptyString());
          response = console.runCommand(STATUS_COMMAND + SOLR_APP);
          assertThat(
              SOLR_APP + " should be ACTIVE after " + START_COMMAND,
              response,
          ...
          response = console.runCommand(STATUS_COMMAND + SOLR_APP);
          assertThat(
              SOLR_APP + " should be INACTIVE after " + STOP_COMMAND,
              response,
  ...
  public void dTestAppAddRemove() throws ApplicationServiceException {
    ...
    systemSubject.execute(
        () -> {
          ...
          assertThat(
              SDK_APP + " should be not be found after " + REMOVE_COMMAND,
              response,
          ...
          response = console.runCommand(STATUS_COMMAND + SDK_APP);
          assertThat(
              SDK_APP + " should be INACTIVE after " + STATUS_COMMAND,
              response,
              containsString(INACTIVE_APP));
        });
  }
