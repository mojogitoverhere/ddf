...
/**
 * Note: Tests prefixed with aRunFirst NEED to run before any other tests. For this reason, we use
 * the @FixMethodOrder(MethodSorters.NAME_ASCENDING) annotation.
 */
...
public class TestConfiguration extends AbstractIntegrationTest {
  ...
  private static final Path SERVER_ENCRYPTION_PROPERTIES =
      Paths.get("etc", "ws-security", "server", "encryption.properties");

  private static final Path SERVER_ENCRYPTION_PROPERTIES_COPY =
      Paths.get("server.encryption.properties.copy");

  private static final Path SERVER_SIGNATURE_PROPERTIES =
      Paths.get("etc", "ws-security", "server", "signature.properties");

  private static final Path SERVER_SIGNATURE_PROPERTIES_COPY =
      Paths.get("server.signature.properties.copy");

  private static final Path ISSUER_ENCRYPTION_PROPERTIES =
      Paths.get("etc", "ws-security", "issuer", "encryption.properties");

  private static final Path ISSUER_ENCRYPTION_PROPERTIES_COPY =
      Paths.get("issuer.encryption.properties.copy");

  private static final Path ISSUER_SIGNATURE_PROPERTIES =
      Paths.get("etc", "ws-security", "issuer", "signature.properties");

  private static final Path ISSUER_SIGNATURE_PROPERTIES_COPY =
      Paths.get("issuer.signature.properties.copy");
  ...
  private static ManagedServiceConfigFile managedServiceNewConfig1 =
      new ManagedServiceConfigFile("ddf.test.itests.platform.TestPlatform.new.1");

  private static ManagedServiceConfigFile managedServiceNewConfig2 =
      new ManagedServiceConfigFile("ddf.test.itests.platform.TestPlatform.new.2");
  ...
  private static ManagedServiceConfigFile configWithNoPid =
      new ManagedServiceConfigFile("ddf.test.itests.platform.TestPlatform.nopid");

  private static ManagedServiceConfigFile invalidConfig =
      new ManagedServiceConfigFile("ddf.test.itests.platform.TestPlatform.invalid");

  private static ManagedServiceConfigFile invalidStartupConfigFile =
      new ManagedServiceConfigFile("ddf.test.itests.platform.TestPlatform.startup.invalid");
  ...
  public void beforeExam() throws Exception {
    try {
      ...
      symbolicLink = Paths.get(ddfHome).resolve("link");
    } catch (Exception e) {
  ...
  public void testExportOnTopOfNestedFile() throws Exception {
    ...
    File file = getDefaultExportDirectory().toFile();
    file.mkdir();
    File fileEtc = getDefaultExportDirectory().resolve("etc").toFile();
  ...
  public void testExportAfterDeletingAConfiguration() throws Exception {
    ...
    configAdmin.getConfiguration(managedServiceNewConfig2.pid, null).delete();

    console.runCommand(EXPORT_COMMAND);

    assertThat(
        "Deleted configuration should not be exported.",
        getPathToExportedConfig(getDefaultExportDirectory(), managedServiceNewConfig2.pid)
            .toFile()
            .isFile(),
        is(false));
  }
  ...
  public void testExportWarningForRelativePathOutsideDdfHome() throws Exception {
    ...
    File systemProperties =
        new File(ddfHome + File.separator + "etc" + File.separator + "system.properties");
  ...
  /**
   * The felix file installer keeps open file handles on files and directories in the etc directory
   * on Windows. This prevents files and directories from being deleted in some of the
   * TestConfiguration itests when running the itests on Windows. This method stops the felix file
   * installer which releases the file handles.
   */
  ...
  private List<String> ingestMetacardsForExport() {
    ...
    String metacardId2 =
        ingest(getFileContent(XML_RECORD_RESOURCE_PATH + "/SimpleXmlMetacard"), "text/xml");
  ...
  private void enableCrls() throws IOException {
    ...
    copyCrlEnabledPropertiesFile(
        CRL_ENABLED_SERVER_ENCRYPTION_PROPERTIES_FILE, SERVER_ENCRYPTION_PROPERTIES);

    backupCrl(SERVER_SIGNATURE_PROPERTIES, SERVER_SIGNATURE_PROPERTIES_COPY);
    copyCrlEnabledPropertiesFile(
        CRL_ENABLED_SERVER_SIGNATURE_PROPERTIES_FILE, SERVER_SIGNATURE_PROPERTIES);
    copyCrl(SERVER_SIGNATURE_DIR_PATH);

    backupCrl(ISSUER_ENCRYPTION_PROPERTIES, ISSUER_ENCRYPTION_PROPERTIES_COPY);
    copyCrlEnabledPropertiesFile(
        CRL_ENABLED_ISSUER_ENCRYPTION_PROPERTIES_FILE, ISSUER_ENCRYPTION_PROPERTIES);
    copyCrl(ISSUER_ENCRYPTION_DIR_PATH);

    backupCrl(ISSUER_SIGNATURE_PROPERTIES, ISSUER_SIGNATURE_PROPERTIES_COPY);
    copyCrlEnabledPropertiesFile(
        CRL_ENABLED_ISSUER_SIGNATURE_PROPERTIES_FILE, ISSUER_SIGNATURE_PROPERTIES);
  ...
  private void copyCrlEnabledPropertiesFile(String source, Path destination) throws IOException {
    FileUtils.copyInputStreamToFile(
        getFileContentAsStream(source), Paths.get(ddfHome).resolve(destination).toFile());
  }
  ...
  private void copyCrl(Path destinationDir) throws IOException {
    ...
    FileUtils.copyInputStreamToFile(
        getFileContentAsStream(CRL_PEM),
  ...
  private void assertExportContents(Path exportDirectory) {
    assertThat(
        getExportSubDirectory(exportDirectory, "system.properties").toFile().exists(), is(true));
    assertThat(
        getExportSubDirectory(exportDirectory, "users.properties").toFile().exists(), is(true));
    assertThat(
        getExportSubDirectory(exportDirectory, "users.attributes").toFile().exists(), is(true));
    assertThat(
        getExportSubDirectory(exportDirectory, "org.codice.ddf.admin.applicationlist.properties")
            .toFile()
            .exists(),
        is(true));
    assertThatDirectoryContains(
        getExportSubDirectory(exportDirectory, "keystores"),
        "serverKeystore.jks",
        "serverTruststore.jks");
    assertThatDirectoryContains(
        getExportSubDirectory(exportDirectory, "pdp", "policies"), "access-policy.xml");
    assertThatDirectoryContains(
        getExportSubDirectory(exportDirectory, "ws-security"),
        "attributeMap.properties",
        "issuer",
        "server");
    assertThatDirectoryContains(
        getExportSubDirectory(exportDirectory, "ws-security", "issuer"),
        "encryption.properties",
        "signature.properties");
    assertThatDirectoryContains(
        getExportSubDirectory(exportDirectory, "ws-security", "server"),
        "encryption.properties",
        "signature.properties");
  }

  private void assertExportCatalog(Path exportPath) {
    String[] metacards = exportPath.toFile().list();
  ...
  private void assertExportContentsWithCrlsEnabled(Path exportDirectory) {
    ...
    assertThatDirectoryContains(
        getExportSubDirectory(exportDirectory, SERVER_SIGNATURE_DIR), CRL_PEM);
    assertThatDirectoryContains(
        getExportSubDirectory(exportDirectory, ISSUER_ENCRYPTION_DIR), CRL_PEM);
    assertThatDirectoryContains(
        getExportSubDirectory(exportDirectory, ISSUER_SIGNATURE_DIR), CRL_PEM);
  }

  private void addConfigurationFile(String fileName, InputStream inputStream) throws IOException {
    FileUtils.copyInputStreamToFile(
        inputStream, getPathToConfigDirectory().resolve(fileName).toFile());
    inputStream.close();
  }

  private void addConfigurationFileAndWaitForSuccessfulProcessing(
      String resourceName, InputStream inputStream) throws IOException {
  ...
  private void addConfigurationFileAndWaitForFailedProcessing(
    ...
    expect("File " + getPathToFailedDirectory().resolve(resourceName).toString() + " exists")
        .within(20, SECONDS)
  ...
  private void assertThatDirectoryContains(Path path, String... fileNames) {
    String[] keystoreFiles = path.toFile().list();
    assertThat("Exported files should not be null.", keystoreFiles, is(notNullValue()));
    assertThat(
        String.format("Files missing in %s directory", path.toString()),
        keystoreFiles,
   ...
   * file.
   *
   * <p>Note: Since we have custom code in the @{link FelixPersistenceStrategy} class to convert
  ...
  private static class ManagedServiceConfigFile {
    ...
    /**
     * Copies the configuration file to the /etc directory and waits for the {@link
     * org.osgi.service.cm.Configuration} object to be initialized with all the values found in the
     * configuration file.
     */
    void addConfigurationFileAndWait(ConfigurationAdmin configAdmin) throws Exception {
      addConfigurationFile();

      expect("Configuration properties for PID " + pid + " to be set")
          .within(20, SECONDS)
          .until(
              new GetConfigurationProperties(configAdmin, "id", pid),
     ...
     */
    private void assertConfigurationPropertiesSet(ConfigurationAdmin configAdmin) throws Exception {
      Dictionary<String, Object> properties =
          new GetConfigurationProperties(configAdmin, "id", pid).call();
      assertThat("No Configuration object exist for PID " + pid, properties, is(notNullValue()));
    ...
    private void assertFileMovedToProcessedDirectory() {
      expect("File to be moved to /etc/processed directory")
          .within(20, SECONDS)
          .until(() -> getPathToProcessedDirectory().exists());
      assertThat(
          String.format(
              "Configuration file %s has not been removed",
    ...
    private void assertFileMovedToFailedDirectory() {
      expect("Waiting for file to be moved to /etc/failed directory")
          .within(20, SECONDS)
    ...
    /**
     * Asserts that the properties in the {@link org.osgi.service.cm.Configuration} object match
     * what' expected.
     */
    void assertPropertiesMatch(
        Dictionary<String, Object> actualProperties, Dictionary<String, Object> expectedProperties)
        throws IOException {
      assertThat(
          "Configuration properties do not match for PID " + pid,
          actualProperties,
          equalToConfigurationProperties(expectedProperties));
    }

    /**
     * Gets the properties from the configuration file using the {@link
     * FelixPersistenceStrategy#read(InputStream)} method. This is used when waiting for the {@link
     * #addConfigurationFileAndWait(ConfigurationAdmin)} when waiting for the {@link
     * org.osgi.service.cm.Configuration} object to be initialized.
     */
    ...
    /**
     * Adds the {@code service.pid} property to the list of expected properties. Needed because MSF
     * configuration files do not have that property.
     */
    ...
    private File getPathToProcessedDirectory() {
      return new File(
          String.format(
              "%s%setc%sprocessed%s", ddfHome, File.separator, File.separator, getResourcePath()));
    }

    private File getPathToFailedDirectory() {
      return new File(
          String.format(
              "%s%setc%sfailed%s", ddfHome, File.separator, File.separator, getResourcePath()));
    }
  ...
  private static class ManagedServiceFactoryConfigFile extends ManagedServiceConfigFile {
     ...
     * {@inheritDoc}
     *
     * <p>Overridden to use {@link ManagedServiceFactoryConfigurationPropertiesEqualTo}.
     */
    @Override
    void assertPropertiesMatch(
        Dictionary<String, Object> actualProperties, Dictionary<String, Object> expectedProperties)
      ...
      assertThat(
          "Configuration properties do not match for PID " + pid,
          actualProperties,
          new ManagedServiceFactoryConfigurationPropertiesEqualTo(factoryPid, expectedProperties));
    }
    ...
    void addConfigurationFileAndWait(ConfigurationAdmin configAdmin) throws Exception {
      ...
      expect("Waiting for Configuration expectedProperties for PID " + pid + " to be set")
          ...
          .until(
              new GetConfigurationProperties(configAdmin, "id", pid),
              new ManagedServiceFactoryConfigurationPropertiesEqualTo(
  ...
  private static class ManagedServiceFactoryConfigurationPropertiesEqualTo
    ...
    private ManagedServiceFactoryConfigurationPropertiesEqualTo(
