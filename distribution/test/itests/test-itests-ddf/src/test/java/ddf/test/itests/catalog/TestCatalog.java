...
public class TestCatalog extends AbstractIntegrationTest {
  ...
  public void testEnforceValidityErrorsOnly() throws Exception {
    ...
    String id3 =
        ingestXmlFromResource(XML_RECORD_RESOURCE_PATH + "/sampleErrorMetacard.xml", false);
  ...
  public void testEnforceValidityWarningsOnly() throws Exception {
    ...
    String id1 =
        ingestXmlFromResource(XML_RECORD_RESOURCE_PATH + "/sampleWarningMetacard.xml", false);
    String id2 = ingestXmlFromResource(XML_RECORD_RESOURCE_PATH + "/sampleCleanMetacard.xml");
  ...
  public void testMetacardIngestNetworkPlugin() throws Exception {
    ...
    final Matcher<Node> hasInitialMetacardTags =
        hasMetacardAttributeXPath("metacard-tags", "GAMMA", "DELTA", "FOXTROT", "resource");
    final Matcher<Node> hasNewDescription = hasMetacardAttributeXPath("description", "None");
    final Matcher<Node> hasNewMetacardTags =
        hasMetacardAttributeXPath("metacard-tags", "ALPHA", "BRAVO", "resource");
    ...
    List<Object> attributeAdjustmentsForRule2 =
        ImmutableList.of("description = None", "metacard-tags = ALPHA, BRAVO, resource");
  ...
  public void testVideoThumbnail() throws Exception {
    ...
    try (InputStream inputStream = IOUtils.toInputStream(getFileContent("sample.mp4"))) {
      ...
      final ValidatableResponse response =
          given()
              .multiPart("file", "sample.mp4", fileBytes, "video/mp4")
              .post(REST_PATH.getUrl())
              .then()
              .statusCode(201);
    }
  ...
  public void testInjectAttributesOnCreate() throws Exception {
    final File file =
        ingestDefinitionJsonWithWaitCondition(
            "injections.json",
            () -> {
              expect("Injectable attributes to be registered")
                  .within(30, TimeUnit.SECONDS)
                  .until(
                      () ->
                          getServiceManager().getServiceReferences(InjectableAttribute.class, null),
                      hasSize(3));
              return null;
            });
    ...
    try {
      ...
      uninstallDefinitionJson(
          file,
          () -> {
            expect("Injectable attributes to be unregistered")
                .within(10, TimeUnit.SECONDS)
                .until(
                    () -> getServiceManager().getServiceReferences(InjectableAttribute.class, null),
                    hasSize(1));
  ...
  private ValidatableResponse executeAdminOpenSearch(String format, String... query) {
    ...
    return given().auth().preemptive().basic("admin", "admin").when().get(url).then();
  }
