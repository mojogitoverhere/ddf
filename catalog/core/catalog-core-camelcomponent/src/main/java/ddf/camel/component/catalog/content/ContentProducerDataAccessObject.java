...
public class ContentProducerDataAccessObject {
  ...
  public void createContentItem(
    ...
    String id =
        fileIdMap.loadAllKeys().contains(key)
            ? String.valueOf(fileIdMap.loadFromPersistence(key))
            : null;
      ...
      CreateStorageRequest createRequest =
          new CreateStorageRequestImpl(
              Collections.singletonList(
                  new ContentItemImpl(
                      Files.asByteSource(ingestedFile), mimeType, ingestedFile.getName(), null)),
              null);

      processHeaders(headers, createRequest, ingestedFile);

      CreateResponse createResponse =
          endpoint.getComponent().getCatalogFramework().create(createRequest);
  ...
  public void processHeaders(
      Map<String, Object> headers, StorageRequest storageRequest, File ingestedFile) {
    Map<String, Serializable> attributeOverrideHeaders =
        new HashMap<>((Map) headers.get(Constants.ATTRIBUTE_OVERRIDES_KEY));
    if (!attributeOverrideHeaders.isEmpty()) {
      storageRequest
          .getProperties()
          .put(Constants.ATTRIBUTE_OVERRIDES_KEY, (Serializable) attributeOverrideHeaders);
    }
