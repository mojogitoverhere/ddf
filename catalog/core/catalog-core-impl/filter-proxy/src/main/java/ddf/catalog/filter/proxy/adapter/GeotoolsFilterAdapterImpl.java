...
public class GeotoolsFilterAdapterImpl implements FilterAdapter, FilterVisitor, ExpressionVisitor {
  ...
  public Object visit(Function expression, Object delegate) {
    ...
    if (expression.getName().equals(ProximityFunction.NAME.getName())) {
      List<Expression> parameters = new ArrayList<>(expression.getParameters());
      if (parameters.size() == ProximityFunction.NUM_PARAMETERS) {

        String propertyName = parameters.remove(0).toString();
        int distance;
        try {
          distance = Integer.parseInt(parameters.remove(0).toString());
        } catch (NumberFormatException e) {
          throw new IllegalArgumentException("Distance parameter was not valid");
        }
        String searchTerm = parameters.remove(0).toString();
  ...
  private ExpressionValues getExpressions(
    ...
    if (expression1 instanceof Function
        && ProximityFunction.NAME
            .getName()
            .equals(((Function) expression1).getFunctionName().getName())
        && expression2 instanceof Literal) {
      return new ExpressionValues(
          (ProximityArguments) expression1.accept(this, delegate),
