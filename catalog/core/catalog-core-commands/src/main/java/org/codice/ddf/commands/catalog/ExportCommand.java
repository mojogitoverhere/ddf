...
public class ExportCommand extends CqlCommands {
    ...
    @Option(name = "--location", description = "Location to list in the tombstone metacard left after export", multiValued = false, required = false)
    ...
    private List<ExportItem> doMetacardExport(/*Mutable,IO*/ZipFile zipFile, Filter filter) {
        ...
        if (delete && seenAssociations.stream()
                .parallel()
                .anyMatch(seenIds::contains)) {
            console.println(
    ...
    static class ExportedMetacard extends MetacardImpl {
        ...
        static final Set<AttributeDescriptor> ATTRIBUTE_DESCRIPTORS =
                ImmutableSet.of(new AttributeDescriptorImpl(EXPORTED_DATA,
                                true,
                                true,
                                false,
                                true,
                        ...
                        new AttributeDescriptorImpl(EXPORTED_LOCATION,
                                true,
                                true,
                                false,
                                false,
                                BasicTypes.STRING_TYPE),
                        new AttributeDescriptorImpl(EXPORTED_TIME,
                                true,
                                true,
                                false,
                                false,
