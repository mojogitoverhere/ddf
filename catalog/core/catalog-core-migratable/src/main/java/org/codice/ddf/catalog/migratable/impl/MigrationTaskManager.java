...
/**
 * Encapsulates all threading state for the catalog migration process and allows inspection of its
 * progress through a public API and the Guava asynchronous call-backs.
 *
 * <p>Repeat calls to exportMetacardQuery({@link List<Result>} results, {@link Long}
 * exportGroupCount) as needed to add file write tasks to the {@link
 * java.util.concurrent.BlockingQueue} of the {@link ExecutorService}. When finished, a call to
 * exportFinish() is required to guarantee that no errors occurred and that the executor will be
 * shutdown properly.
 */
class MigrationTaskManager implements AutoCloseable {
  ...
  /**
   * The creation of a task manager requires the passing of migratable configurations so that task
   * behavior can be explicitly defined (i.e. where to export, how many metacards per file, and
   * other useful data). Also initializes the {@link MigrationFileWriter} and threading services.
   *
   * @param config Configuration object to use for customizing catalog migration process.
   */
  public MigrationTaskManager(
      @NotNull final CatalogMigratableConfig config,
   ...
   *
   * <p>Must be called after all invocations to exportMetacardQuery. This will attempt to gracefully
   * shutdown the executor service and handle any exceptions that might have occured in the
   * asynchronous threads.
   *
   * <p>If you are using {@link MigrationTaskManager}; that is, you have constructed an instance,
   * you must call this method to gracefully release resources and shutdown. Can also be used within
   * the scope of a try-with-resources block to utilize {@link AutoCloseable}.
   *
  ...
  /**
   * Creates a sublist from the given results and a destination file for metacards based on the
   * {@link CatalogMigratableConfig} object used by this task manager. An asynchronous task is
   * started for exporting the metacards and is deferred to the appropriate instance of {@link
   * MigrationFileWriter} for processing.
   *
   * @param results The results of a catalog query that need to be exported.
   * @param exportGroupCount The group or page number of the query. This value is not monitored or
   *     audited for repeats and is strictly for record keeping by naming the resulting export files
   *     appropriately.
  ...
  public void exportMetacardQuery(final List<Result> results, long exportGroupCount)
    ...
    for (int i = 0; i < results.size(); i += catalogConfig.getExportCardsPerFile()) {
      final List<Result> fileResults =
          results.subList(i, Math.min((i + catalogConfig.getExportCardsPerFile()), results.size()));

      final File exportFile =
          catalogConfig.getExportPath().resolve(makeFileName(exportGroupCount, i)).toFile();
  ...
  private String makeFileName(long queryPageNumber, int fileNumber) {
    return String.format(
        "%s_%d_%d", catalogConfig.getExportFilePrefix(), queryPageNumber, fileNumber);
  }
}
