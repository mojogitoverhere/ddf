...
public class MigrationFileWriter {
   ...
   *
   * <p>If the directory does exist, work based on the assumption it was cleaned by the parent
   * process in the management logic of inport and export. There is no point to delegate low level
   * directory operations to each and every migratable when they can be abstracted into the base
   * logic.
   *
   * @param exportPath The path representing the directory to create.
   * @throws MigrationException thrown if the export directory doesn't exist and could not be
   *     created
   */
  ...
  public void writeMetacards(File exportFile, final List<Result> results) throws IOException {
    ...
    try (ObjectOutputStream objectStream = createStream(exportFile); ) {
