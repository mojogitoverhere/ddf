...
public class MigrationTaskManagerTest {
  ...
  /**
   * The following values are arbitrary to some degree but must remain consistent throughout the
   * test.
   *
   * <p>TEST_GROUP_COUNT: Independent of the other values and strictly for file naming
   * TEST_FILE_SIZE: Number of metacards to be written per file TEST_RESULTS_LIST_SIZE: Size of the
   * list we're passing as if it came from a catalog query. Note that the math yields 3 total files
   * (i.e. 7/3 = 2 full files + 1 for remainder). This is an important relationship for the
   * integrity of the test. TEST_FUTURES_LIST_SIZE: Independent, arbitrary number of futures to
  ...
  @Captor private ArgumentCaptor<File> argFileCaptor;

  @Captor private ArgumentCaptor<List<Result>> argResultCaptor;

  @Mock private MigrationFileWriter mockFileWriter;
  ...
  /**
   * For testing, the export path can be arbitrary. Our {@link CatalogMigratableConfig} object is
   * real and initilized to defaults. The {@link MigrationTaskManager} is the class under test and
   * is loaded with a REAL list of MOCK future objects, otherwise we will get a null pointer
   * exception in the foreach loop of exportFinish().
   *
   * <p>The exception to this rule is the final test method where we examine exportMetacardQuery()
  ...
  public void testExportFinishedInterruptedShutdown() throws Exception {
    when(mockExecutor.awaitTermination(1L, TimeUnit.MINUTES)).thenThrow(InterruptedException.class);
  ...
  public void testExportMetacardQuerySubmit() throws Exception {
    ...
    verify(mockFileWriter, times(expectedCallsToWriteMetacards))
        .writeMetacards(argFileCaptor.capture(), argResultCaptor.capture());
    ...
    try {
      ...
      for (int i = 0; i < expectedResults.size(); i++) {
        assertThat(expectedResults.get(i), containsInAnyOrder(actualResults.get(i).toArray()));
      }
  ...
  private List<File> generateTestFiles() {
    ...
    testFiles.add(config.getExportPath().resolve("catalogExport_12_0").toFile());
    testFiles.add(config.getExportPath().resolve("catalogExport_12_3").toFile());
    testFiles.add(config.getExportPath().resolve("catalogExport_12_6").toFile());
    return testFiles;
  }

  /**
   * We need to generate the original list of results. We feed this to exportMetacardQuery() in the
   * {@link MigrationTaskManager}.
   *
  ...
  private List<List<Result>> generateExpectedResults(List<Result> originalResults) {
    ...
    for (int i = 0; i < originalResults.size(); i += TEST_FILE_SIZE) {
      expectedResults.add(
          originalResults.subList(i, Math.min((i + TEST_FILE_SIZE), originalResults.size())));
    }
