...
public class MetacardsMigratableTest {
  ...
  private static final DescribableBean DESCRIBABLE_BEAN =
      new DescribableBean(VERSION, ID, TITLE, DESCRIPTION, ORGANIZATION);

  @Captor private ArgumentCaptor<QueryRequest> argQueryRequest;
  ...
  @Mock private MigrationTaskManager mockTaskManager;
  ...
  public void setup() throws Exception {
    ...
    when(mockFilterBuilder.attribute(Metacard.ANY_TEXT).is().like().text("*"))
  ...
  public void constructorWithNullDescribable() {
    new MetacardsMigratable(
        null, mockFramework, mockFilterBuilder, mockFileWriter, config, mockTaskManager);
  }

  @Test(expected = IllegalArgumentException.class)
  public void constructorWithNullFramework() {
    new MetacardsMigratable(
        DESCRIBABLE_BEAN, null, mockFilterBuilder, mockFileWriter, config, mockTaskManager);
  }

  @Test(expected = IllegalArgumentException.class)
  public void constructorWithNullFilterBuilder() {
    new MetacardsMigratable(
        DESCRIBABLE_BEAN, mockFramework, null, mockFileWriter, config, mockTaskManager);
  }

  @Test(expected = IllegalArgumentException.class)
  public void constructorWithNullFileWriter() {
    new MetacardsMigratable(
        DESCRIBABLE_BEAN, mockFramework, mockFilterBuilder, null, config, mockTaskManager);
  }

  @Test(expected = IllegalArgumentException.class)
  public void constructorWithNullConfig() {
    new MetacardsMigratable(
        DESCRIBABLE_BEAN, mockFramework, mockFilterBuilder, mockFileWriter, null, mockTaskManager);
  }

  @Test(expected = IllegalArgumentException.class)
  public void constructorWithNullTaskManager() {
    new MetacardsMigratable(
        DESCRIBABLE_BEAN, mockFramework, mockFilterBuilder, mockFileWriter, config, null);
  }
  ...
  public void exportUsesRightQuerySettings() throws Exception {
    ...
    assertThat(
        argQueryRequest.getValue().getQuery().getPageSize(), is(config.getExportQueryPageSize()));
    assertThat(argQueryRequest.getValue().getQuery().requestsTotalResultsCount(), is(false));
  }
  ...
  public void exportWhenResultSetIsNull() throws Exception {
    when(mockFramework.query(any()).getResults()).thenReturn(null);
    migratable.export(EXPORT_PATH);
  }

  @Test
  public void exportWhenResultSetIsEmpty() throws Exception {
    when(mockFramework.query(any()).getResults()).thenReturn(new ArrayList<>());
    migratable.export(EXPORT_PATH);
  ...
  public void exportWhenResultSetEqualToPageSize() throws Exception {
    ...
    assertThat(
        "Invalid page size",
        argQueryRequest.getAllValues().get(0).getQuery().getPageSize(),
        is(config.getExportQueryPageSize()));
    assertThat(
        "Invalid start index for second query",
        startIndices.get(1),
  ...
  public void exportStopsWhenExportMetacardFails() throws Exception {
    ...
    doThrow(new RuntimeException()).when(mockTaskManager).exportMetacardQuery(result1, 1);
  ...
  public void exportWhenProviderQueryFails() throws Exception {
    ...
    MetacardsMigratable migratable =
        new MetacardsMigratable(
            DESCRIBABLE_BEAN,
            mockFramework,
            mockFilterBuilder,
  ...
  private List<Integer> setupProviderResponses(final List<Result> result1, List<Result> result2)
    ...
    QueryResponse response = mock(QueryResponse.class);
    when(response.getResults()).thenReturn(result1).thenReturn(result2);
    ...
    when(mockFramework.query(any()))
        .thenAnswer(
            (invocation) -> {
              ...
              responses.add(request.getQuery().getStartIndex());
              return response;
            });
