...
public class SizeValidatorTest {
  ...
  private void validateNoErrors(final Attribute attribute, final long min, final long max) {
    final Optional<AttributeValidationReport> reportOptional =
        getReportOptional(attribute, min, max);
    assertThat(reportOptional.isPresent(), is(false));
  }

  private void validateWithErrors(
      final Attribute attribute, final long min, final long max, final int expectedErrors) {
    final Optional<AttributeValidationReport> reportOptional =
        getReportOptional(attribute, min, max);
    assertThat(reportOptional.get().getAttributeValidationViolations(), hasSize(expectedErrors));
  }

  private Optional<AttributeValidationReport> getReportOptional(
      final Attribute attribute, final long min, final long max) {
