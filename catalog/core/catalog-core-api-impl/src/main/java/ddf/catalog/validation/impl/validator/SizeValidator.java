 ...
 *
 * <p>Is capable of validating the sizes of {@link CharSequence}s, {@link Collection}s, {@link
 * Map}s, and arrays.
 */
public class SizeValidator implements AttributeValidator {
   ...
   *
   * <p>The minimum must be non-negative and the maximum must be greater than the minimum.
   *
    ...
    for (final Serializable value : attribute.getValues()) {
      ...
      if (!checkSize(size)) {
        final String violationMessage =
            String.format("%s size must be between %d and %d", name, min, max);
  ...
  public boolean equals(Object o) {
    ...
    return new EqualsBuilder().append(min, validator.min).append(max, validator.max).isEquals();
  }

  @Override
  public int hashCode() {
    return new HashCodeBuilder(29, 37).append(min).append(max).toHashCode();
  }
}
