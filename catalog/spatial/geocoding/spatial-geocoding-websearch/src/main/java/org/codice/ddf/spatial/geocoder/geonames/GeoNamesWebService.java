...
public class GeoNamesWebService implements GeoCoder {
  ...
  public GeoResult getLocation(String location) {
    ...
    String urlStr =
        String.format(
            "%s://%s/searchJSON?q=%s&username=%s",
            GEONAMES_PROTOCOL, GEONAMES_API_ADDRESS, location, USERNAME);
      ...
      if (result instanceof JSONObject) {
        ...
        if (geonames != null && geonames.size() > 0) {
          ...
          if (firstResult != null) {
            ...
            return GeoResultCreator.createGeoResult(
                (String) firstResult.get(PLACENAME_KEY), lat, lon, adminCode, population);
          }
        }
      }
    }
