...
public class XstreamTreeWriter {
  ...
  XstreamTreeWriter(
      PathTrackingWriter writer, PathTracker tracker, XstreamPathValueTracker pathValues) {
    this.writer = writer;
  ...
  void startVisit(final String node) {
    ...
    for (Path path : pathValues.getPaths()) {
      ...
      if (value != null) {
        ...
        if (currentPath.isAncestor(path) && path.toString().endsWith(node)) {
  ...
  private String normalizeNode(String node) {
    return NORMALIZE_NODE.matcher(node).replaceAll("");
  }
