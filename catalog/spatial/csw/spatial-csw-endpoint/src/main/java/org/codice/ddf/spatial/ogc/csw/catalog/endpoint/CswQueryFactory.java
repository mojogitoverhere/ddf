...
public class CswQueryFactory {
  ...
  private CswRecordMapperFilterVisitor buildFilter(QueryConstraintType constraint)
    ...
    CswRecordMapperFilterVisitor visitor =
        new CswRecordMapperFilterVisitor(metacardType, metacardTypes, attributeRegistry);
    Filter filter = null;
    if (constraint != null) {
  ...
  private SortBy buildSort(SortByType sort) throws CswException {
    ...
    if (!attributeRegistry.lookup(sortBy.getPropertyName().getPropertyName()).isPresent()
      ...
      throw new CswException(
          "Property " + sortBy.getPropertyName().getPropertyName() + " is not a valid SortBy Field",
