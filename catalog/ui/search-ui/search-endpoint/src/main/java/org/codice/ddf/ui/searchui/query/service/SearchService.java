...
public class SearchService {
  ...
  private Set<String> getSourceIds(String sources) {
    ...
    if (StringUtils.equalsIgnoreCase(sources, LOCAL_SOURCE)) {
      ...
      try {
        sourceInfo =
            searchController.getFramework().getSourceInfo(new SourceInfoRequestEnterprise(true));
      } catch (SourceUnavailableException e) {
        LOGGER.debug(
            "Exception while getting source status. Defaulting to all sources. "
                + "This could include unavailable sources.",
            e);
      }
  ...
  private Query createQuery(
      Filter filter, Long startIndexLng, Long countLng, String sortStr, Long maxTimeoutLng) {
    // default values
