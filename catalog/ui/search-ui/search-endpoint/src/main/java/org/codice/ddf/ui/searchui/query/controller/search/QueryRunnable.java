...
public abstract class QueryRunnable implements Runnable {
  ...
  protected String extractQueryWkt(Filter filter) {
    String wkt = "";
    try {
      wkt = searchController.getFilterAdapter().adapt(filter, new WktExtractionFilterDelegate());
  ...
  protected QueryResponse queryCatalog(
      String sourceId,
    ...
    try {
      ...
      LOGGER.info(
          "Error executing query. {}. Set log level to DEBUG for more information", e.getMessage());
  ...
  private QueryResponse getEmptyResponse(String sourceId) {
    ...
    QueryRequest queryRequest =
        new QueryRequestImpl(null, false, Collections.singletonList(sourceId), null);
