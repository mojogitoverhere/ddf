...
public class SearchController {
   ...
   * @param serverSession
   */
  public synchronized void publishResults(
      String id, Map<String, Object> jsonData, ServerSession serverSession) {
    String channel = "/" + id;
    ...
    bayeuxServer.createChannelIfAbsent(
        channel,
        (ConfigurableServerChannel.Initializer) serverChannel -> serverChannel.setPersistent(true));
  ...
  public void executeQuery(
      final SearchRequest request, final ServerSession session, final Subject subject) {
    ...
    if (shouldNormalizeRelevance(request)) {
      ...
      solrIndexFuture =
          executorService.submit(new FilteringSolrIndexCallable(request, filterAdapter));
    } else {
    ...
    if (!cacheDisabled) {
      ...
      cacheFuture =
          executorService.submit(
              new CacheQueryRunnable(
                  this, request, subject, search, session, results, solrIndexFuture));
    } else {
