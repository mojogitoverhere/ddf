...
define([
    ...
    return Marionette.LayoutView.extend(Decorators.decorate({
        ...
        generateDetailsShow: function(){
            this.detailsShow.show(new ShowAttributeView({
                model: new DropdownModel(),
                selectionInterface: this.options.selectionInterface
            }), {
                replaceElement: true
            });
            this.listenTo(this.detailsShow.currentView.model, 'change:value', this.handleShowAttribute);
        },
        generateDetailsHide: function(){
            this.detailsHide.show(new HideAttributeView({
                model: new DropdownModel(),
                selectionInterface: this.options.selectionInterface
            }), {
                replaceElement: true
            });
            this.listenTo(this.detailsHide.currentView.model, 'change:value', this.handleHideAttribute);
        },
