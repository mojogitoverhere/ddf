...
public class WorkspacePolicyExtension implements PolicyExtension {
  ...
  private static Function<KeyValuePermission, KeyValuePermission> remapKeys(
    ...
    return permission ->
        new KeyValuePermission(mapping.get(permission.getKey()), permission.getValues());
  }
  ...
  private Stream<KeyValuePermission> overrides() {
    return Stream.of(
        new KeyValuePermission(systemUserAttribute, ImmutableSet.of(systemUserAttributeValue)));
  }
  ...
  public KeyValueCollectionPermission isPermittedMatchAll(
    ...
    Stream<KeyValuePermission> remapped =
        permissions.stream().filter(byKeys(keyMapping.keySet())).map(remapKeys(keyMapping));

    boolean matched =
        concat(overrides(), remapped).filter(subject::implies).findFirst().isPresent();
  ...
  private boolean isOwnerPermission(
      List<KeyValuePermission> permissions, CollectionPermission subject) {
    Optional<KeyValuePermission> ownerPerm =
        permissions
            .stream()
            .filter(keyValuePermission -> keyValuePermission.getKey().equals(Core.METACARD_OWNER))
            .findAny();

    if (ownerPerm.isPresent()) {
      KeyValuePermission ownerEmailPermission =
          new KeyValuePermission(Constants.EMAIL_ADDRESS_CLAIM_URI, ownerPerm.get().getValues());
