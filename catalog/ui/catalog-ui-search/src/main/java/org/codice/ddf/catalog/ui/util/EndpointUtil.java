...
public class EndpointUtil {
  ...
  static {
    ...
    try {
      solrQueryRowSize =
          Integer.valueOf(System.getProperty("solr.query.rows", Integer.toString(2_000_000_000)));
  ...
  public Map<String, Result> getMetacards(
      List<String> writableSources, Collection<String> ids, Filter tagFilter)
      throws UnsupportedQueryException, SourceUnavailableException, FederationException {
  ...
  public Map<String, Result> getMetacards(String attributeName, Collection<String> ids, String tag)
      throws UnsupportedQueryException, SourceUnavailableException, FederationException {
    return getMetacards(
        null, attributeName, ids, filterBuilder.attribute(Metacard.TAGS).is().like().text(tag));
  }

  public Map<String, Result> getMetacards(
      List<String> writableSources, String attributeName, Collection<String> ids, Filter tagFilter)
      throws UnsupportedQueryException, SourceUnavailableException, FederationException {
    if (ids.isEmpty()) {
      return new HashMap<>();
    }
    ...
    if (CollectionUtils.isEmpty(writableSources)) {
      queryRequest =
          new QueryRequestImpl(
              new QueryImpl(
                  queryFilter, 1, -1, SortBy.NATURAL_ORDER, false, TimeUnit.SECONDS.toMillis(10)),
              false);
    } else {
      queryRequest =
          new QueryRequestImpl(
  ...
  public static <T, R extends Comparable> Comparator<T> compareBy(Function<T, R> getField) {
    return (T o1, T o2) -> getField.apply(o1).compareTo(getField.apply(o2));
  }
