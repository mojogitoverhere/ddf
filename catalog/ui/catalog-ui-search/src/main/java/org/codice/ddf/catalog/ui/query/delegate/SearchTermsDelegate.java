...
public class SearchTermsDelegate extends SimpleFilterDelegate<Set<SearchTerm>> {
  ...
  @Override
  public <S> Set<SearchTerm> defaultOperation(
      Object property, S literal, Class<S> literalClass, Enum operation) {
    return Collections.emptySet();
  }

  @Override
  public Set<SearchTerm> propertyIsLike(
      String propertyName, String pattern, boolean isCaseSensitive) {
    String[] patternWords = pattern.toLowerCase().split("[\\s\\p{Punct}&&[^*]]+");

    return Arrays.stream(patternWords).map(SearchTerm::new).collect(Collectors.toSet());
  }
  ...
  @Override
  public Set<SearchTerm> propertyIsInProximityTo(
      String propertyName, Integer distance, String searchTerms) {
    return Collections.emptySet();
  }

  @Override
  public Set<SearchTerm> propertyIsNotInProximityTo(
      String propertyName, Integer distance, String searchTerms) {
    return Collections.emptySet();
  }
}
