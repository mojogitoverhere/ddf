...
/**
 * HistorianPolicyPlugin prevents anyone without the {@link HistorianPolicyPlugin#HISTORY_ROLE} from
 * modifying a {@link MetacardVersion} in any way.
 */
public class HistorianPolicyPlugin implements PolicyPlugin {
  ...
  private final Predicate<Metacard> isMetacardHistory =
      (tags) ->
          tags != null
              && tags.getTags() != null
              && tags.getTags().contains(MetacardVersion.VERSION_TAG);

  private final Predicate<Metacard> isDeletedMetacard =
      (tags) ->
          tags != null
              && tags.getTags() != null
              && tags.getTags().contains(DeletedMetacard.DELETED_TAG);
  ...
  public PolicyResponse processPreCreate(Metacard input, Map<String, Serializable> properties)
    ...
    return new PolicyResponseImpl(
        new HashMap<>(), Collections.singletonMap(ROLE_CLAIM, Collections.singleton(HISTORY_ROLE)));
  }
  ...
  public PolicyResponse processPreDelete(
      ...
      throws StopProcessingException {
    if (!metacards.stream().anyMatch(isHistoryOrDeleted)) {
      /* not modifying history, proceed */
      return new PolicyResponseImpl();
    }
    return new PolicyResponseImpl(
