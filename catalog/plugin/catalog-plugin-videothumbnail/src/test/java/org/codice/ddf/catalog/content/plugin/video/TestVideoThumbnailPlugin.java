...
public class TestVideoThumbnailPlugin {
  ...
  private byte[] getCreatedItemThumbnail(final String videoFile) throws Exception {
    ...
    doReturn(Collections.singletonList(mockContentItem))
    ...
    final CreateStorageResponse processedCreateResponse =
        videoThumbnailPlugin.process(mockCreateResponse);

    return (byte[])
        processedCreateResponse
            .getCreatedContentItems()
            .get(0)
            .getMetacard()
  ...
  public void testUpdatedItemGifThumbnail() throws Exception {
    ...
    final UpdateStorageResponse processedUpdateResponse =
        videoThumbnailPlugin.process(mockUpdateResponse);

    final byte[] thumbnail =
        (byte[])
            processedUpdateResponse
                .getUpdatedContentItems()
                .get(0)
                .getMetacard()
                .getAttribute(Metacard.THUMBNAIL)
