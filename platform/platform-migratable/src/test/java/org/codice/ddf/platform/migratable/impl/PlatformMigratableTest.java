...
public class PlatformMigratableTest {
  ...
  private static final Path APPLICATION_LIST =
  ...
  public void testExportValidRelativePaths() throws Exception {
    ...
    PlatformMigratable platformMigratable =
        new PlatformMigratable(DESCRIBABLE_BEAN, mockMigratableUtil);
  ...
  public void testExportWarningsReturnedWhenExportingKeystoreTruststoreAppList() throws Exception {
    ...
    doAnswer(new MigrationWarningAnswer(expectedMigrationWarning1))
        .when(mockMigratableUtil)
        .copyFileFromSystemPropertyValue(
            eq(KEYSTORE_SYSTEM_PROP),
            eq(exportDirectory),
            Matchers.<Collection<MigrationWarning>>any());
    MigrationWarning expectedMigrationWarning2 = new MigrationWarning("warning 2");
    doAnswer(new MigrationWarningAnswer(expectedMigrationWarning2))
        .when(mockMigratableUtil)
        .copyFileFromSystemPropertyValue(
            eq(TRUSTSTORE_SYSTEM_PROP),
            eq(exportDirectory),
            Matchers.<Collection<MigrationWarning>>any());
    MigrationWarning expectedMigrationWarning3 = new MigrationWarning("warning 3");
    doAnswer(new MigrationWarningAnswer(expectedMigrationWarning3))
        .when(mockMigratableUtil)
        .copyFile(
            eq(APPLICATION_LIST),
            eq(exportDirectory),
    ...
    assertThat(
        migrationMetadata.getMigrationWarnings(), containsInAnyOrder(expectedWarnings.toArray()));
  }
  ...
  public void testExportWarningReturnedWhenExportingWsSecurity() throws Exception {
    ...
    doAnswer(new MigrationWarningAnswer(expectedMigrationWarning))
        .when(mockMigratableUtil)
        .copyDirectory(
            eq(WS_SECURITY_DIR_REL_PATH),
            eq(exportDirectory),
            Matchers.<Collection<MigrationWarning>>any());
    ...
    // Verify
    assertThat(
        migrationMetadata.getMigrationWarnings().contains(expectedMigrationWarning), is(true));
  }
  ...
  public void testExportExceptionThrownWhenCopyingFile() throws Exception {
    ...
    doThrow(MigrationException.class)
        .when(mockMigratableUtil)
        .copyFile(
            any(Path.class), eq(exportDirectory), Matchers.<Collection<MigrationWarning>>any());
    PlatformMigratable platformMigratable =
        new PlatformMigratable(DESCRIBABLE_BEAN, mockMigratableUtil);
  ...
  public void testExportExceptionThrownWhenCopyingFileFromSystemPropertyValue() throws Exception {
    ...
    doThrow(MigrationException.class)
        .when(mockMigratableUtil)
        .copyFileFromSystemPropertyValue(
            any(String.class), eq(exportDirectory), Matchers.<Collection<MigrationWarning>>any());
    PlatformMigratable platformMigratable =
        new PlatformMigratable(DESCRIBABLE_BEAN, mockMigratableUtil);
  ...
  private void assertSystemPropertiesFilesExport(MigratableUtil mockMigratableUtil) {
    verify(mockMigratableUtil)
        .copyFile(eq(SYSTEM_PROPERTIES_REL_PATH), eq(exportDirectory), Matchers.any());
    verify(mockMigratableUtil)
        .copyFile(eq(USERS_PROPERTIES_REL_PATH), eq(exportDirectory), Matchers.any());
    verify(mockMigratableUtil)
        .copyFile(eq(USERS_ATTRIBUTES_REL_PATH), eq(exportDirectory), Matchers.any());
  }
