...
public class PlatformMigratable extends DescribableBean implements ConfigurationMigratable {
  ...
  private static final Path DDF_METACARD_ATTRIBUTE_RULESET =
      Paths.get("etc", "pdp", "ddf-metacard-attribute-ruleset.cfg");

  private static final Path DDF_USER_ATTRIBUTE_RULESET =
  ...
  private void exportSystemFiles(
      Path exportDirectory, Collection<MigrationWarning> migrationWarnings) {
    LOGGER.debug(
        "Exporting system files: [{}], [{}], [{}], and [{}]",
  ...
  private void exportKeystores(
      Path exportDirectory, Collection<MigrationWarning> migrationWarnings) {
    LOGGER.debug("Exporting keystore and truststore...");
    migratableUtil.copyFileFromSystemPropertyValue(
        KEYSTORE_SYSTEM_PROP, exportDirectory, migrationWarnings);
    migratableUtil.copyFileFromSystemPropertyValue(
        TRUSTSTORE_SYSTEM_PROP, exportDirectory, migrationWarnings);
  }
}
