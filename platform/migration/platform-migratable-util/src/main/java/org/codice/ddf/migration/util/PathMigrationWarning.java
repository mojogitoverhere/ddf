...
public class PathMigrationWarning extends MigrationWarning {
  private static final String BASE_WARNING =
      "therefore, it will not be included in the export.  "
  ...
  private static final String JAVA_FILE_PROPERTY_ABSOLUTE_PATH_WARNING =
      "The value for property [%s] in file [%s] is set to a path [%s] that %s; " + BASE_WARNING;
  ...
  public PathMigrationWarning(String systemProperty, Path path, String reason) {
    super(
        String.format(
            SYSTEM_PROPERTY_ABSOLUTE_PATH_WARNING, systemProperty, path.toString(), reason));
  }
   ...
   * @param path path that is absolute and cannot be exported
   */
  public PathMigrationWarning(
      Path propertiesFilePath, String systemProperty, Path path, String reason) {
    super(
        String.format(
            JAVA_FILE_PROPERTY_ABSOLUTE_PATH_WARNING,
