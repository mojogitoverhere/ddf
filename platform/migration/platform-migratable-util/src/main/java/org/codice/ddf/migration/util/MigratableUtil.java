...
public class MigratableUtil {
  ...
  /**
   * Copies a file to a destination directory. The file to copy must be a relative path under {@code
   * ddf.home}, and its path must not contain any symbolic link, otherwise the file will not be
   * copied and a {@link MigrationWarning} will be returned.
   *
   * @param sourceFile relative path to the file to copy
   * @param exportDirectory root directory where the file will be copied. If the file' relative path
   *     included any directories, those will be re-created under this directory.
   * @param warnings any warnings generated during this operation (e.g., source file outside of
   *     {@code ddf.home}) will be added to this collection
   * @throws MigrationException thrown if a file system error prevents the file to be copied
   */
  public void copyFile(
      @NotNull Path sourceFile,
      @NotNull Path exportDirectory,
    ...
    copy(
        ...
        () ->
            isSourceMigratable(
                sourceFile, (reason) -> new PathMigrationWarning(sourceFile, reason), warnings));
  }

  /**
   * Recursively copies a directory to a destination directory. The directory to copy must be a
   * relative path under {@code ddf.home}, and its path must not contain any symbolic link,
   * otherwise the directory will not be copied and a {@link MigrationWarning} will be returned.
   *
  ...
  public void copyDirectory(
    ...
    try {
      if (isSourceMigratable(
          source, (reason) -> new PathMigrationWarning(source, reason), warnings)) {
        FileUtils.copyDirectory(source.toFile(), exportDirectory.resolve(source).toFile());
      }
    } catch (IOException e) {
      String message =
          String.format(
              "Unable to copy [%s] to [%s].", source.toString(), exportDirectory.toString());
   ...
   * directory. The file to copy must be a relative path under {@code ddf.home}, and its path must
   * not contain any symbolic link, otherwise the file will not be copied and a {@link
   * MigrationWarning} will be returned.
   *
   * @param systemProperty name of the {@link System} property that contains the path to the source
   *     file
   * @param exportDirectory root directory where the file will be copied. If the file' relative path
   *     included any directories, those will be re-created under this directory.
   * @param warnings any warnings generated during this operation (e.g., source file outside of
  ...
  public void copyFileFromSystemPropertyValue(
    ...
    notEmpty(
        source,
        String.format("Source path property [%s] is invalid: [%s]", systemProperty, source));
   ...
   * MigrationWarning} will be returned.
   *
   * @param propertyFilePath path to the Java properties file that contains the path to the source
   *     file to copy
   * @param javaProperty name of the property inside the Java properties file that contains the path
   *     to the source file
   * @param exportDirectory path to the destination
   * @param warnings any warnings generated during this operation (e.g., source file outside of
   *     {@code ddf.home}) will be added to this collection
   */
  public void copyFileFromJavaPropertyValue(
      @NotNull Path propertyFilePath,
    ...
    notEmpty(
        source, String.format("Source path property [%s] is invalid: [%s]", javaProperty, source));
    ...
    copy(
        ...
        () ->
            isSourceMigratable(
                sourcePath,
                (reason) ->
                    new PathMigrationWarning(propertyFilePath, javaProperty, sourcePath, reason),
                warnings));
  }
   ...
   */
  public String getJavaPropertyValue(@NotNull Path propertyFilePath, @NotNull String javaProperty)
      throws MigrationException {
    notNull(propertyFilePath, "Java properties file cannot be null");
  ...
  private void copy(
    ...
    try {
      ...
      String message =
          String.format(
              "Unable to copy [%s] to [%s]", sourceFile.toString(), exportDirectory.toString());
      LOGGER.info(message, e);
      throw new ExportMigrationException(message, e);
    }
  }
  ...
  private Properties readPropertiesFile(Path propertiesFile) throws MigrationException {
    ...
    try (InputStream inputStream = new FileInputStream(propertiesFile.toString())) {
      ...
      String message =
          String.format("Unable to read properties file [%s]", propertiesFile.toString());
      LOGGER.info(message, e);
      throw new ExportMigrationException(message, e);
    }
  }

  private boolean isSourceMigratable(
      Path source,
