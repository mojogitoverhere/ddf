...
abstract class PropertyValueConverter {
  ...
  private static final Pattern PROPERTY_VALUE_PATTERN =
      Pattern.compile(String.format("%s|%s|%s", SINGLE_VALUE_REGEX, ARRAY_REGEX, VECTOR_REGEX));
   ...
   *
   * <p>Note that values will be converted only if their format follows the Felix format for values
   * which can be "value", ["value1", "value2", ...] or ("value1", "value2", ...). Any value that
   * doesn't match that format will just be output as-is and not converted.
   *
   * @param propertyValue property value to convert. Assumes that the value provided has been
   *     trimmed, starts with a quote ("), square bracket ([) or parenthesis and is not {@code
   *     null}.
   * @param output {@link StringBuilder} to append the value to. Assumes that this is never {@code
   *     null}.
   */
  ...
  /**
   * Converts a single property value to a new format if needed. If the conversion fails or the
   * value cannot or does not need to be converted, the original value should be appended back.
   * Unless part of the conversion logic, the returned value should not be quoted.
   *
   * @param value value to convert. The value will never have any leading or trailing white spaces
   *     or quotes (") and will never be {@code null}.
   * @param output {@link StringBuilder} to use to output the converted value. Will never be {@code
   *     null}.
   */
  protected abstract void convertSingleValue(String value, StringBuilder output);

  private void processMultipleValues(
      String openingBracket, String propertyValues, String closingBracket, StringBuilder output) {
    output.append(openingBracket);

    String propertyValuesWithoutBrackets =
        removeSurroundingCharacters(propertyValues, openingBracket, closingBracket);
