...
class PropertyConverter implements Consumer<String> {
  ...
  private static final Pattern PROPERTY_PATTERN =
      Pattern.compile(
          String.format(
              "^%s=%s%s", PROPERTY_NAME_REGEX, PROPERTY_TYPE_REGEX, PROPERTY_VALUE_REGEX));

  private static final String NEW_LINE = "\r\n";

  private static Map<String, PropertyValueConverter> valueConverters =
      ImmutableMap.of("f", new FloatValueConverter(), "d", new DoubleValueConverter());
  ...
  /**
   * Converts the line provided if necessary and appends the line back to the {@code filteredOutput}
   * provided in the constructor.
   *
   * <p>This method will delegate the property value conversion to the proper {@link
   * PropertyValueConverter} based on the type of property and whether a property converter can be
   * found for that type.
   *
   * <p>Before calling the property converter, this method will remove all leading and trailing
   * white spaces and make sure that the value starts with either a quote ("), square bracket ([) or
   * opening parenthesis and is not {@code null}.
   *
  ...
  public void accept(String line) {
    ...
    if (matcher.matches()) {
      ...
      if (propertyType != null
          ...
          && valueConverters.containsKey(propertyType.toLowerCase())) {
        valueConverters.get(propertyType.toLowerCase()).convert(propertyValue, newLine);
      } else {
        LOGGER.debug("Property value {} for line {} doesn't need conversion", propertyValue, line);
