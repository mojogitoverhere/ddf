...
public class ConfigurationFilesPoller implements FileAlterationListener {
  ...
  public void register(ChangeListener listener) {
    ...
    fileAlterationObserver =
        new FileAlterationObserver(
            configurationDirectoryPath.toAbsolutePath().toString(),
            new SuffixFileFilter(fileExtension));
    fileAlterationObserver.addListener(this);
    watchService = new FileAlterationMonitor(POLLING_INTERVAL, fileAlterationObserver);
    try {
      watchService.start();
    } catch (Exception e) {
      logStackAndMessageSeparately(
          e, "Failed to start, 'Platform :: Migration' must be restarted: ");
    }
  }
  ...
  public void onFileCreate(File file) {
    try {
      ...
      LOGGER.debug(
          "Notifying [{}] of creation for file [{}].",
          changeListener.getClass().getName(),
          file.getPath());
