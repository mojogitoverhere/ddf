...
public class ConfigurationAdminMigrationTest {
  ...
  private static final Path CONFIG_FILE_PATH =
      Paths.get("/root/etc/exported/etc/" + CONFIG_PID + CONFIGURATION_FILE_EXTENSION);
  ...
  @Mock private DirectoryStream<Path> configurationDirectoryStream;

  @Mock private DirectoryStream<Path> failedDirectoryStream;

  @Mock private Iterator<Path> failedDirectoryStreamIterator;

  @Mock private File exportedDirectory;

  @Mock private ConfigurationFileFactory configurationFileFactory;

  @Mock private ConfigurationFilesPoller configurationFilePoller;

  @Mock private ConfigurationAdmin configurationAdmin;

  @Mock private Iterator<Path> configFilesIterator;

  @Mock private ConfigurationFile configFile1;

  @Mock private ConfigurationFile configFile2;

  @Mock private Configuration configuration;

  @Mock private MigrationWarning configStatus1;

  @Mock private MigrationWarning configStatus2;

  @Mock private Path failedDirectory;
  ...
  public void testConstructorWithNullDirectoryStream() {
    new ConfigurationAdminMigration(
        null,
  ...
  public void testInitMovesFilesToProcessedDirectory() throws Exception {
    ...
    when(Files.move(CONFIG_PATH1, PROCESSED_DIRECTORY_PATH, REPLACE_EXISTING))
        .thenReturn(PROCESSED_DIRECTORY_PATH);

    when(Files.move(CONFIG_PATH2, PROCESSED_DIRECTORY_PATH, REPLACE_EXISTING))
        .thenReturn(PROCESSED_DIRECTORY_PATH);

    ConfigurationAdminMigration configurationAdminMigrator =
        new ConfigurationAdminMigration(
            configurationDirectoryStream,
            PROCESSED_DIRECTORY_PATH,
            FAILED_DIRECTORY_PATH,
  ...
  public void testInitMovesFilesWithAnInvalidTypeToFailedDirectory() throws Exception {
    ...
    when(configurationFileFactory.createConfigurationFile(CONFIG_PATH1))
        .thenThrow(new ConfigurationFileException(""));
    when(configurationFileFactory.createConfigurationFile(CONFIG_PATH2))
        .thenThrow(new ConfigurationFileException(""));
    when(Files.move(CONFIG_PATH1, PROCESSED_DIRECTORY_PATH, REPLACE_EXISTING))
    ...
    verifyStatic();
    Files.move(
        CONFIG_PATH1, CONFIG_FILE_IN_FAILED_DIRECTORY.resolve(CONFIG_FILE_PATH1), REPLACE_EXISTING);
    Files.move(
        CONFIG_PATH2, CONFIG_FILE_IN_FAILED_DIRECTORY.resolve(CONFIG_FILE_PATH2), REPLACE_EXISTING);
    verify(configurationDirectoryStream).iterator();
    verify(configurationFileFactory).createConfigurationFile(CONFIG_PATH1);
    verify(configurationFileFactory).createConfigurationFile(CONFIG_PATH2);
    verify(configurationDirectoryStream).close();
  ...
  public void testInitStillProcessesFilesWhenTheyCannotBeMovedToProcessedDirectory()
    ...
    when(Files.newDirectoryStream(any(Path.class), any(String.class)))
        .thenReturn(failedDirectoryStream);
  ...
  @Test
  public void testInitStillProcessesFilesWhenTheyCannotBeMovedToFailedDirectory() throws Exception {
  ...
  public void testNotifyWhenFileIsReprocessed() throws Exception {
    ...
    verifyStatic();
    Files.move(
        Paths.get("/root/etc/" + CONFIG_FILE_PATH1),
        Paths.get(PROCESSED_DIRECTORY_PATH.resolve(CONFIG_FILE_PATH1).toString()),
        REPLACE_EXISTING);
  }
  ...
  public void testNotifyFailsToMoveFileToProcessedDirectory() throws Exception {
    ...
    when(configurationFileFactory.createConfigurationFile(CONFIG_PATH1)).thenReturn(configFile1);
    when(Files.move(CONFIG_PATH1, CONFIG_FILE_IN_PROCESSED_DIRECTORY)).thenThrow(new IOException());
  ...
  public void testGetFailedConfigurationFilesNoFilesInFailedDirectory() throws Exception {
    ...
    assertThat(
        "The failed directory does not contain the correct number of files",
  ...
  @Test
  public void testExport() throws IOException, InvalidSyntaxException, ConfigurationFileException {

    when(configurationAdmin.listConfigurations(anyString()))
        .thenReturn(new Configuration[] {configuration});
    when(configurationFileFactory.createConfigurationFile((Dictionary<String, Object>) anyObject()))
  ...
  private ConfigurationAdminMigration createConfigurationAdminMigratorWithNoFiles() {
    ...
    return new ConfigurationAdminMigration(
        configurationDirectoryStream,
        PROCESSED_DIRECTORY_PATH,
        FAILED_DIRECTORY_PATH,
        configurationFileFactory,
  ...
  private void setUpTwoConfigFileIterator(DirectoryStream<Path> stream) {
    ...
    when(configFilesIterator.next()).thenReturn(CONFIG_PATH1).thenReturn(CONFIG_PATH2);
  }
