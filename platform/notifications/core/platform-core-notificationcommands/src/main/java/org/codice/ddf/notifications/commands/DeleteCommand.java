...
@Command(
  scope = "notifications",
  name = "delete",
  description = "Allows users to delete notifications for a specified user."
)
public class DeleteCommand extends OsgiCommandSupport {
  ...
  @Option(
    ...
    aliases = {"-u"},
    multiValued = false,
    description = "The user to delete notifications for."
  )
  String userId = null;
  ...
  @Option(
    ...
    aliases = {"-id"},
    multiValued = false,
    description = "The id to delete notification for."
  )
  String id = null;
  ...
  protected Object doExecute() throws Exception {
      ...
      if (persistentStore != null) {
        if (StringUtils.isNotBlank(id)) {
          ...
          try {
            ...
            LOGGER.info(
                "PersistenceException during deletion of notifications for user {}", userId, e);
          }
        }
      } else {
    ...
    console.println(
        CYAN_CONSOLE_COLOR + "Deleted " + numDeleted + " notifications" + DEFAULT_CONSOLE_COLOR);
