...
public class SecurityMigratableTest {
  ...
  private static final Path SERVER_ENCRYPTION_PROPERTIES_PATH =
      Paths.get("etc", "ws-security", "server", "encryption.properties");

  private static final Path SERVER_SIGNATURE_PROPERTIES_PATH =
      Paths.get("etc", "ws-security", "server", "signature.properties");

  private static final Path ISSUER_ENCRYPTION_PROPERTIES_PATH =
      Paths.get("etc", "ws-security", "issuer", "encryption.properties");

  private static final Path ISSUER_SIGNATURE_PROPERTIES_PATH =
      Paths.get("etc", "ws-security", "issuer", "signature.properties");

  private static final Path EXPECTED_SERVER_SIGNATURE_CRL_PATH =
      Paths.get("ddf", "crl", "serverSignature", "crl.pem");

  private static final Path EXPECTED_SERVER_ENCRYPTION_CRL_PATH =
      Paths.get("ddf", "crl", "serverEncryption", "crl.pem");

  private static final Path EXPECTED_ISSUER_SIGNATURE_CRL_PATH =
      Paths.get("ddf", "crl", "issuerSignature", "crl.pem");

  private static final Path EXPECTED_ISSUER_ENCRYPTION_CRL_PATH =
      Paths.get("ddf", "crl", "issuerEncryption", "crl.pem");
  ...
  public void testExportValidRelativePaths() throws Exception {
    ...
    SecurityMigratable securityMigratable =
        new SecurityMigratable(DESCRIBABLE_BEAN, mockMigratableUtil);
  ...
  public void testWarningsReturned() throws Exception {
    ...
    doAnswer(new MigrationWarningAnswer(expectedWarning))
        .when(migratableUtil)
        .copyDirectory(
            eq(PDP_POLICIES_DIR_REL_PATH),
            eq(EXPORT_DIRECTORY),
            Matchers.<Collection<MigrationWarning>>any());

    SecurityMigratable securityMigratable =
        new SecurityMigratable(DESCRIBABLE_BEAN, migratableUtil);
  ...
  public void testExportExceptionThrownWhenCopyingDirectory() throws Exception {
    MigratableUtil mockMigratableUtil = mock(MigratableUtil.class);
    doThrow(MigrationException.class)
        .when(mockMigratableUtil)
        .copyDirectory(
            any(Path.class), eq(EXPORT_DIRECTORY), Matchers.<Collection<MigrationWarning>>any());
    SecurityMigratable securityMigratable =
        new SecurityMigratable(DESCRIBABLE_BEAN, mockMigratableUtil);
  ...
  public void testExportCrlIsNull() {
    ...
    // Verify
    verify(mockMigratableUtil, never())
        .copyFile(
            eq(EXPECTED_SERVER_ENCRYPTION_CRL_PATH),
            eq(EXPORT_DIRECTORY),
  ...
  private void assertCrlExport(MigratableUtil mockMigratableUtil) {
    ...
    verify(mockMigratableUtil)
        .copyFile(
            eq(EXPECTED_SERVER_SIGNATURE_CRL_PATH),
            eq(EXPORT_DIRECTORY),
            anyCollectionOf(MigrationWarning.class));
    verify(mockMigratableUtil)
        .copyFile(
            eq(EXPECTED_ISSUER_ENCRYPTION_CRL_PATH),
            eq(EXPORT_DIRECTORY),
            anyCollectionOf(MigrationWarning.class));
    verify(mockMigratableUtil)
        .copyFile(
            eq(EXPECTED_ISSUER_SIGNATURE_CRL_PATH),
            eq(EXPORT_DIRECTORY),
            anyCollectionOf(MigrationWarning.class));
  }
