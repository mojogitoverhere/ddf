...
public abstract class PkiTools {
  ...
  /**
   * If input is a character array, return the character array. If input is null, return a zero
  ...
  /**
   * Get the host name or DNS name associated with the machine running the JVM. This method is
   * public so client code can easily check the name and decide if it should be used in the
   * generated certificate.
   *
   * @return String. Hostname of this machine. Hostname should be the same as the machine's DNS
   *     name.
   */
  ...
  public static PrivateKey pemToPrivateKey(String keyString) {
    try {
      ...
      throw new CertificateGeneratorException(
          "Could not convert String to Private Key", e.getCause());
    }
  }
