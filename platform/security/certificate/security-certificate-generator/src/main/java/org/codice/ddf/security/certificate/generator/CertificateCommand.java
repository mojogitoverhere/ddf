...
public class CertificateCommand {
   ...
   * if 0 arguments or more than 1 argument.
   *
   * @param args
   */
  public static void main(String args[]) {
    ...
    if (args[0].trim().equalsIgnoreCase("-cn")) {
      configureDemoCert(args[1].trim());
    } else {
      String[] dn = Arrays.stream(args[1].split("[,]")).map(String::trim).toArray(String[]::new);
   ...
   *     e.g. {@code configureDemoCertWithDN("cn=John Whorfin", "o=Yoyodyne", "l=San Narciso",
   *     "st=California", "c=US")}
   * @return the string used as the common name in the new certificate
   */
