...
public class ApplicationServiceImpl implements ApplicationService, ServiceListener {
  ...
  /**
   * Given a {@code Set} of {@code Feature}s, returns the subset of {@code Features}s that are not
   * installed
   *
   * @param features The {@code Set} of {@link Feature}s from which to construct the subset of
   *     {@code Feature}s that are not installed
   * @return A {@code Set} of {@code Feature}s that are not installed that is a sub-set of the
   *     <i>features</i> {@code Feature}s {@code Set} parameter
   */
  ...
  public synchronized void startApplication(Application application)
    ...
    try {
      ...
      if (!autoInstallFeatures.isEmpty()) {
        Set<String> autoFeatureNames =
            autoInstallFeatures.stream().map(Feature::getName).collect(Collectors.toSet());
        for (Feature feature : autoInstallFeatures) {
          if (featuresService.isInstalled(feature)) {
            autoFeatureNames.remove(feature.getName());
          } else {
            for (Dependency dependency : feature.getDependencies()) {
              if (!application.getName().equals(dependency.getName())
                  && getApplicationNames().contains(dependency.getName())) {
        ...
        if (!autoFeatureNames.isEmpty()) {
          featuresService.installFeatures(
              autoFeatureNames, EnumSet.of(Option.NoAutoRefreshBundles));
