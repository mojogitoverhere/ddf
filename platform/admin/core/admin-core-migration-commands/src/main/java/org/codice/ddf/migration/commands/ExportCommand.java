...
@Command(
  scope = MigrationCommands.NAMESPACE,
  name = "export",
  description =
...
public class ExportCommand extends MigrationCommands {
  ...
  @Argument(
    ...
    description = "Path to directory to store export",
    required = false,
    multiValued = false
  )
  ...
  ExportCommand(
      ConfigurationMigrationService configurationMigrationService, Security security, Path path) {
    ...
    try {
      Collection<MigrationWarning> migrationWarnings =
          security.runWithSubjectOrElevate(
              () -> configurationMigrationService.export(exportDirectory));
