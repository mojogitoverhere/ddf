...
public class ExportCommandTest {
  ...
  private static final String ERROR_RED =
      Ansi.ansi().a(Attribute.RESET).fg(Ansi.Color.RED).toString();

  private static final String WARNING_YELLOW =
      Ansi.ansi().a(Attribute.RESET).fg(Ansi.Color.YELLOW).toString();

  private static final String INFO_WHITE =
      Ansi.ansi().a(Attribute.RESET).fg(Ansi.Color.WHITE).toString();

  private static final String SUCCESS_GREEN =
      Ansi.ansi().a(Attribute.RESET).fg(Ansi.Color.GREEN).toString();
  ...
  @Mock private DirectoryStream<Path> mockDirectoryStream;

  @Mock private PrintStream mockConsole;

  @Mock private Iterator<Path> mockIterator;

  @Mock private ConfigurationMigrationService mockConfigurationMigrationService;

  @Mock private Security security;

  @Mock private Path mockDefaultExportDirectory;

  @Test
  public void testDoExecuteReportWarnings() throws Exception {
    // Setup
    when(security.runWithSubjectOrElevate(any(Callable.class)))
        .thenAnswer(this::delegateToCallable);
    when(mockConfigurationMigrationService.export(mockDefaultExportDirectory))
        .thenReturn(CONFIG_STATUS_MSGS);
    ExportCommand exportCommand =
        new ExportCommandUnderTest(mockConfigurationMigrationService, mockDefaultExportDirectory);
    ...
    assertThat(
        values.get(1), is(String.format(STARTING_EXPORT_MESSAGE, mockDefaultExportDirectory)));
    ...
    assertThat(values.get(5), is(String.format(FAILED_EXPORT_MESSAGE, mockDefaultExportDirectory)));
  }
  ...
  public void testDoExecuteWithExportDirectoryArgument() throws Exception {
    ...
    when(security.runWithSubjectOrElevate(any(Callable.class)))
        .thenAnswer(this::delegateToCallable);
    when(mockConfigurationMigrationService.export(exportDirectoryPath))
        .thenReturn(ImmutableList.of());

    ExportCommand exportCommand =
        new ExportCommandUnderTest(mockConfigurationMigrationService, exportDirectoryPath);
  ...
  public void testDoExecuteErrorOccurred() throws Exception {
    ...
    when(security.runWithSubjectOrElevate(any(Callable.class)))
        .thenThrow(
            new InvocationTargetException(new MigrationException(MIGRATION_EXCEPTION_MESSAGE)));
    ExportCommand exportCommand =
        new ExportCommandUnderTest(mockConfigurationMigrationService, mockDefaultExportDirectory);
  ...
  public void testDoExecuteWhenSecurityExceptionIsThrown() throws Exception {
    // Setup
    when(security.runWithSubjectOrElevate(any(Callable.class)))
        .thenThrow(new SecurityServiceException(INSUFFICIENT_PRIVILEGES_MESSAGE));
    ExportCommand exportCommand =
        new ExportCommandUnderTest(mockConfigurationMigrationService, mockDefaultExportDirectory);
