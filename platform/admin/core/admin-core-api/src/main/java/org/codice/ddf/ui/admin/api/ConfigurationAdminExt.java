...
public class ConfigurationAdminExt {
   ...
   */
  private static final IdGetter PID_GETTER =
      new IdGetter() {
   ...
   */
  private static final IdGetter FACTORY_PID_GETTER =
      new IdGetter() {
        public String[] getIds(MetaTypeInformation metaTypeInformation) {
  ...
  public List<Map<String, Object>> listServices(String serviceFactoryFilter, String serviceFilter) {
    ...
    try {
      ...
      List<Map<String, Object>> metatypeList =
          addMetaTypeNamesToMap(getPidObjectClasses(), serviceFilter, Constants.SERVICE_PID);
      ...
      for (Map<String, Object> service : serviceFactoryList) {
        ...
        for (Map<String, Object> metatype : metatypeList) {
          if (metatype.get(MAP_ENTRY_ID) != null
              && service.get(MAP_ENTRY_ID) != null
  ...
  private void addConfigurationData(Map<String, Object> service, Configuration[] configs) {
    for (Configuration config : configs) {
      ...
      metatypeList
          .stream()
          .filter(metatype -> AttributeDefinition.PASSWORD == (Integer) metatype.get("type"))
          .forEach(
              metatype -> {
      ...
      Map<String, Object> pluginDataMap =
          getConfigurationPluginData(
              configData.get(MAP_ENTRY_ID).toString(), Collections.unmodifiableMap(configData));
  ...
  List<Map<String, Object>> getServices(
      String serviceClass, String serviceFilter, boolean ocdRequired)
      throws InvalidSyntaxException {
    List<Map<String, Object>> serviceList = new ArrayList<Map<String, Object>>();

    // find all ManagedServiceFactories to get the factoryPIDs
    ServiceReference[] refs =
        this.getBundleContext().getAllServiceReferences(serviceClass, serviceFilter);
  ...
  public List<Map<String, Object>> addMetaTypeNamesToMap(
      final Map ocdCollection, final String filterSpec, final String type) {
    Filter filter = null;
    if (filterSpec != null) {
      try {
