...
/**
 * ValidationDecorator - a specific decorator for the {@link AdminModule} that adds support for
 * validation.
 */
public class ValidationDecorator extends Decorator {
  ...
  /**
   * Determine if an {@link AdminModule} is valid. NOTE: a valid module cannot contain absolute
   * URIs.
   *
   * @return
   */
  public boolean isValid() {
    return isValidURI(getJSLocation())
        && isValidURI(getCSSLocation())
        && isValidURI(getIframeLocation());
  }
}
