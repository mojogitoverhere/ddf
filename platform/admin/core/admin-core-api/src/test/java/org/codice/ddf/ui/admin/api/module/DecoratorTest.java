...
public class DecoratorTest {
  ...
  public void testCompareTo() {
    doReturn("test").when(module).getName();
    Decorator proxy = new ValidationDecorator(module);
    assertThat("modules are the same if the have the same name", proxy.compareTo(module), is(0));
  }
  ...
  public void testSort() {
    ...
    assertThat("modules are sorted lexographically by name", modules.get(0).getName(), is("hello"));
    assertThat("modules are sorted lexographically by name", modules.get(1).getName(), is("world"));
  }
}
